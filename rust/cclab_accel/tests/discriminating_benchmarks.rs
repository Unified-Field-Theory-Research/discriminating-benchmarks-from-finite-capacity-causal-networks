...
use cclab_accel::{
    ...
    DBM008FinalConditionalCertificate, Paper14SkeletonCertificate, HIDDEN_IMPORT_BLOCKERS,
...
#[test]
fn dbm005_references_paper13_intake_without_validation_import() {
    let compatibility = DBM005Paper13IntakeCompatibility::canonical();
    assert!(compatibility.closes_dbm005());
    assert!(compatibility.outcome_set.closes_dbm004());
    assert_eq!(compatibility.paper13_frozen_commit, PAPER13_FROZEN_COMMIT);
    assert_eq!(
        compatibility.paper13_formal_endpoint,
        PAPER13_FORMAL_ENDPOINT
    );
    assert_eq!(
        compatibility.paper13_final_certificate,
        PAPER13_FINAL_CERTIFICATE
    );
    assert!(compatibility.finite_compatibility_map);
    assert!(compatibility.final_certificate_referenced_only);
    assert!(compatibility.formal_endpoint_referenced_only);
    assert!(compatibility.no_validation_readout_closure_imported);
    assert!(compatibility.no_evidence_intake_promotion_imported);
    assert!(compatibility.no_empirical_adequacy_imported);
    assert!(compatibility.no_observed_catalog_recovery_imported);
    assert!(compatibility.no_prediction_success_imported);
    assert!(compatibility.no_falsification_closure_imported);
}

#[test]
fn dbm005_skeleton_still_keeps_paper14_theorem_open() {
    let skeleton = Paper14SkeletonCertificate::through_dbm005();
    assert!(skeleton.dbm001_upstream_binding_closed);
    assert!(skeleton.dbm002_finite_benchmark_record_closed);
    assert!(skeleton.dbm003_target_comparator_regime_closed);
    assert!(skeleton.dbm004_outcome_uncertainty_audit_closed);
...
#[test]
fn dbm006_defines_stability_and_coarse_graining_without_promotion() {
    let stability = DBM006StabilityCoarseGraining::canonical();
    assert!(stability.closes_dbm006());
    assert!(stability.paper13_compatibility.closes_dbm005());
    assert!(stability.finite_stability_map);
    assert!(stability.finite_coarse_graining_map);
    assert!(stability.benchmark_local_stability);
    assert!(stability.coarse_graining_preserves_audit_boundary);
    assert!(stability.stability_does_not_assert_validation);
    assert!(stability.coarse_graining_does_not_assert_physical_promotion);
    assert!(stability.no_prediction_success_imported);
    assert!(stability.no_falsification_closure_imported);
    assert!(stability.no_empirical_adequacy_imported);
}

#[test]
fn dbm006_skeleton_still_keeps_paper14_theorem_open() {
    let skeleton = Paper14SkeletonCertificate::through_dbm006();
    assert!(skeleton.dbm001_upstream_binding_closed);
    assert!(skeleton.dbm002_finite_benchmark_record_closed);
    assert!(skeleton.dbm003_target_comparator_regime_closed);
    assert!(skeleton.dbm004_outcome_uncertainty_audit_closed);
...
#[test]
fn dbm008_final_certificate_closes_conditional_theorem_only() {
    let certificate = DBM008FinalConditionalCertificate::canonical();
    assert!(certificate.closes_dbm008());
    assert!(certificate.no_hidden_import_audit.closes_dbm007());
    assert!(certificate.final_conditional_certificate_closed);
    assert!(certificate.conditional_interface_theorem_only);
    assert!(certificate.no_benchmark_pass_claim);
    assert!(certificate.no_external_evidence_promotion);
    assert!(certificate.no_physical_validation_claim);
    assert!(certificate.no_empirical_adequacy_claim);
    assert!(certificate.no_physical_nature_claim);
    assert!(certificate.no_unified_field_theory_claim);
    assert!(certificate
        .claim_boundary
        .all_physical_and_benchmark_claims_remain_false());
}
...
#[test]
fn final_certificate_closes_paper14_theorem_contract() {
    let skeleton = Paper14SkeletonCertificate::final_conditional();
    assert!(skeleton.dbm001_upstream_binding_closed);
    assert!(skeleton.dbm002_finite_benchmark_record_closed);
    assert!(skeleton.dbm003_target_comparator_regime_closed);
    assert!(skeleton.dbm004_outcome_uncertainty_audit_closed);
...
#[test]
fn docs_record_final_closure_and_physical_claims_false() {
    ...
    assert_eq!(active_obligation(), "CLOSED");
    assert!(state.contains("\"active_obligation\": \"CLOSED\""));
    assert!(config.contains("\"active_obligation\": \"CLOSED\""));
    assert!(paper_config.contains("\"active_obligation\": \"CLOSED\""));
    ...
    assert!(state.contains("\"dbm008_final_conditional_certificate_closed\": true"));
    assert!(state.contains("\"discriminating_benchmarks_theorem_closed\": true"));
    ...
    assert!(state_md.contains("discriminating benchmarks theorem is closed"));
    assert!(theorem.contains("DBM-008"));
    assert!(theorem.contains("Final conditional certificate"));
